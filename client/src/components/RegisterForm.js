...
export default function RegisterForm() {
  ...
  const handleDataAvailability = async (data) => {
    try {
      ...
      if (errorMessage.startsWith("Username")) setValidation({ ...validation, username: errorMessage });
      if (errorMessage.startsWith("Email")) setValidation({ ...validation, email: errorMessage });
    }
  };
  ...
  useEffect(() => {
    ...
    if (username.length > 0) {
      ...
      if (username[lastIndex] === " ") {
        setData(prevData => ({ ...prevData, username: username.slice(0, lastIndex) }));
      }
    }
  }, [data.username]);
  ...
  useEffect(() => {
    ...
    ? setValidation(prevState => ({ ...prevState, passwordConfirmation: "Passwords doesn't match" }))
    : setValidation(prevState => ({ ...prevState, passwordConfirmation: "" }));
  }, [data.password, data.passwordConfirmation]);
