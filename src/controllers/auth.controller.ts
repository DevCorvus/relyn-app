...
const refreshToken = async (req: Request, res: Response) => {
  ...
  const refreshTokenExists: boolean = await Token.exists({ refreshToken: token });
  if (!refreshTokenExists) return res.sendStatus(403);
  
  try {
    const { _id, remember } = verify(token, REFRESH_TOKEN_SECRET) as PayloadInterface;
...
const csrfToken = async (req: Request, res: Response) => {
  ...
  if (!req.session.csrfToken) req.session.csrfToken = randomBytes(100).toString("base64");
