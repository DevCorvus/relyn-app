...
const index = async (req: Request, res: Response) => {
  ...
  const posts = await paginate({ type: "post", page, username, search, sortBy });
...
const store = async (req: Request, res: Response) => {
  ...
  if (imageUrlFromClient) {
    ...
    if (val.isURL(imageUrlFromClient) && regexIsImage.test(imageUrlFromClient)) {
...
const like = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const postAlreadyLiked = post.likes.filter(like => like.username === username).length > 0;
    if (postAlreadyLiked) {
