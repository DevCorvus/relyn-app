...
export const tokenSlayer = ((slayTime: number) => {
  ...
  return {
    ...
    add: (token: Token) => {
      ...
      setTimeout(() => {
        tokensInLine = tokensInLine.filter(tokenInLine => tokenInLine !== token);
      }, slayTime);
    }
  }
})(ACCESS_TOKEN_EXPIRATION);
...
export const deleteAllRefreshToken = async (userId: string): Promise<boolean> => {
  try {
...
export const generateTokens = async (_id: string, remember: boolean = false): Promise<TokenResponseInterface> => {
