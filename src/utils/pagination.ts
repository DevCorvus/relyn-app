...
export const paginate = async ({ type, page, postId, username, search, sortBy }: PaginateInterface) => {
  let limit: number;
  ...
  try {    
    if (type === "post") {
      ...
      if (username && search && sortBy) {
        posts = await Post
          .find({ username, body: { $regex: search, $options: "i" } }, queryConfig)
          .sort(sortParam)
          .limit(limit)
          .skip(skipIndex)
          .lean();
        ...
        posts = await Post
          .find({ body: { $regex: search, $options: "i" } }, queryConfig)
          .sort(sortParam)
          .limit(limit)
          .skip(skipIndex)
          .lean();
